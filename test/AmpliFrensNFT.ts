...
describe("NFT", async () => {
  ...
  describe("Transfer", async () => {
    it("Should revert if the recipient already has one NFT", async () => {
      ...
      await expect(nftContract.transferNFT(accounts[0].address, accounts[1].address, 2)).to.be.revertedWith(
  ...
  describe("Royalties", async () => {
    ...
    it("Should revert if new receiver is address(0)", async () => {
      await expect(nftContract.setDefaultRoyalty(ethers.utils.hexZeroPad("0x", 20), 1000)).to.be.reverted;
    });
